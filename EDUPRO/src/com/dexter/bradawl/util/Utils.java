...
public class Utils
{
	...
	/**
	 * Checks if bradawl is currently setup properly.
	 * */
	public boolean checkAppSetup()
	{
		...
		try
		{
			...
			stored_procedure = con.prepareCall("{call checkSetup_sp(?)}");
			stored_procedure.setInt(1, -1);
			
			stored_procedure.registerOutParameter(1, Types.INTEGER);
			
			stored_procedure.execute();
			
			output = stored_procedure.getInt(1);
			if(output == 1)
				ret = true;
			else
				ret = false;
