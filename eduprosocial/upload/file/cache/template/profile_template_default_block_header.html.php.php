...
?>
	<div<?php if (Phpfox ::getService('profile')->timeline()): ?> class="profile_timeline_header_holder"<?php endif; ?>>
		<div class="profile_header<?php if (Phpfox ::getService('profile')->timeline()): ?> profile_timeline_header<?php endif;  if (empty ( $this->_aVars['aUser']['cover_photo'] )): ?> no_cover_photo<?php endif; ?>">
...
<?php if (Phpfox ::getUserParam('profile.can_change_cover_photo')): ?>
						<li><a href="#" onclick="$('#cover_section_menu_drop').toggle(); return false;"><?php if (empty ( $this->_aVars['aUser']['cover_photo'] )):  echo Phpfox::getPhrase('user.add_a_cover');  else:  echo Phpfox::getPhrase('user.change_cover');  endif; ?></a>
...
<?php endif; ?>
				...
				<h1 style="width:400px;"><a href="<?php echo Phpfox::getLib('phpfox.url')->makeUrl($this->_aVars['aUser']['user_name']); ?>" title="<?php echo Phpfox::getLib('phpfox.parse.output')->clean($this->_aVars['aUser']['full_name']); ?>"><?php echo Phpfox::getLib('phpfox.parse.output')->shorten(Phpfox::getLib('phpfox.parse.output')->split(Phpfox::getLib('phpfox.parse.output')->clean($this->_aVars['aUser']['full_name']), 30), 50, '...'); ?></a><?php if (count((array)$this->_aVars['aBreadCrumbs'])):  $this->_aPhpfoxVars['iteration']['link'] = 0;  foreach ((array) $this->_aVars['aBreadCrumbs'] as $this->_aVars['sLink'] => $this->_aVars['sCrumb']):  $this->_aPhpfoxVars['iteration']['link']++;  if ($this->_aPhpfoxVars['iteration']['link'] == 1): ?><span class="profile_breadcrumb">&#187;</span><a href="<?php echo $this->_aVars['sLink']; ?>"><?php echo $this->_aVars['sCrumb']; ?></a><?php endif;  endforeach; endif; ?></h1>
